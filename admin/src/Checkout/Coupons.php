...
class Coupons {
    ...
    public static function get_coupon_field_position() {
		$sidebar_hook = array(
			...
			'callback' => array( __CLASS__, 'render_sidebar_coupon_row' ),
		);

		$before_payment_hook = array(
        ...
        return apply_filters( 'Flexify_Checkout/Checkout/Coupon_Field_Position', array(
            'sidebar' => array(
                ...
                'hook' => $sidebar_hook['hook'],
				'callback' => $sidebar_hook['callback'],
            ),
            'before_payment_title' => array(
                'title' => esc_html__( 'Before payment method', 'flexify-checkout-for-woocommerce' ),
                'hook' => $before_payment_hook['hook'],
				'callback' => $before_payment_hook['callback'],
            ),
            'before_payment_and_sidebar' => array(
                'title' => esc_html__( 'Before payment method and sidebar', 'flexify-checkout-for-woocommerce' ),
				'hooks' => array( $before_payment_hook, $sidebar_hook ),
            ),
        ));
    }
	...
	/**
	 * Render the coupon form as a row inside the order review sidebar table
	 *
	 * @since 6.0.0
	 * @return void
	 */
	public static function render_sidebar_coupon_row() {
		?>
	...
	public static function render_coupon_field() {
		$positions = self::get_coupon_field_position();
		$selected = Admin_Options::get_setting('render_coupon_field_hook');

		if ( ! isset( $positions[ $selected ] ) ) {
			return;
		}

		$value = $positions[ $selected ];

		// Support a single hook (legacy structure) or multiple hooks per position.
		$hooks = isset( $value['hooks'] ) ? $value['hooks'] : array( $value );

		foreach ( $hooks as $entry ) {
			if ( ! empty( $entry['hook'] ) && ! empty( $entry['callback'] ) ) {
				add_action( $entry['hook'], $entry['callback'], 10 );
			}
		}
	}
}
