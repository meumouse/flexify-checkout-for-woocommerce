...
class Fields {
	...
	public static function custom_override_checkout_fields( $fields ) {
		...
		if ( empty( $fields['shipping']['shipping_address_2']['label'] ) ) {
			$fields['shipping']['shipping_address_2']['label'] = __( 'Apartment, suite, unit etc.', 'flexify-checkout-for-woocommerce' );
		}
	...
	public static function custom_fields_on_user_profile( $fields ) {
		...
		$new_fields['billing']['title'] = __( 'Billing address', 'flexify-checkout-for-woocommerce' );
	...
	public static function custom_override_default_fields( $fields ) {
		...
		$fields['address_2']['label'] = __( 'Apartment, suite, unit etc.', 'flexify-checkout-for-woocommerce' );
	...
	public static function modify_form_field_html( $field, $key, $args, $value ) {
		$field_required = __( 'This field is required', 'flexify-checkout-for-woocommerce' );
		$valid_number = __( 'Please enter a valid phone number', 'flexify-checkout-for-woocommerce' );
	...
	public static function render_inline_errors( $field = '', $key = '', $args = array(), $value = '', $country = '' ) {
		...
		if ( $field_is_required || ( isset( $args['class'] ) && $args['class'] === 'required-field' ) ) {
			$message = sprintf( __( '%s is a required field.', 'flexify-checkout-for-woocommerce' ), esc_html( $field_label ) );
		...
		if ( $field_is_required && $value ) {
			if ( 'country' === $field_type && property_exists( WC()->countries, 'country_exists' ) && WC()->countries && ! WC()->countries->country_exists( $value ) ) {
				...
				$message = sprintf( __( '\'%s\' is not a valid country code.', 'flexify-checkout-for-woocommerce' ), esc_html( $field_label ) );
				$custom  = true;
			}
			...
			if ( 'billing_postcode' === $key && ! \WC_Validation::is_postcode( $value, $country ) ) {
				switch ( $country ) {
					case 'IE':
						...
						$message = sprintf( __( '%1$s is not valid. You can search for the correct Eircode <a target="_blank" href="%2$s">here</a>.', 'flexify-checkout-for-woocommerce' ), esc_html( $field_label ), 'https://finder.eircode.ie' );
						$custom  = true;
						break;
					default:
						/* translators: %s: field name */
						$message = sprintf( __( '%s is not a valid postal code.', 'flexify-checkout-for-woocommerce' ), esc_html( $field_label ) );
						$custom  = true;
						break;
				}
			}
			...
			if ( strpos( $key, 'phone' ) !== false ) {
				if ( ! \WC_Validation::is_phone( $value ) || Admin_Options::get_setting('enable_ddi_phone_field') && ! Utils::is_valid_phone( $value ) ) {
					$message = sprintf( __( '%s is not a valid phone number.', 'flexify-checkout-for-woocommerce' ), esc_html( $field_label ) );
					$custom  = true;
				}
			}
			...
			if ( strpos( $key, 'billing_cpf' ) !== false && ! Utils::validate_cpf( $value ) || ( isset( $args['class'] ) && 'validate-cpf-field' === $args['class'] && ! Utils::validate_cpf( $value ) ) ) {
				$message = sprintf( __('The %s provided is not valid.', 'flexify-checkout-for-woocommerce'), esc_html( $field_label ) );
				$custom  = true;
			}
			...
			if ( 'email' === $field_type && ! is_email( $value ) || ( isset( $args['class'] ) && 'validate-email-field' === $args['class'] && ! is_email( $value ) ) ) {
				$message = sprintf( __('%s is not a valid email address.', 'flexify-checkout-for-woocommerce'), esc_html( $field_label ) );
				$custom  = true;
			}
			...
			if ( 'email' === $field_type && ! is_user_logged_in() && email_exists( $value ) ) {
				 ...
				 * @return string
				 */
				$message = apply_filters( 'Flexify_Checkout/Checkout/Fields/Email_Exists_Message', sprintf( __( 'An account is already registered with this email address. <a href="#" data-login>Would you like to log in to your account?</a>', 'flexify-checkout-for-woocommerce' ), '' ) );
				$message_type = 'info';
			}
		}
	...
	public function validate_required_account_password( $data, $errors ) {
		...
		if ( $password === '' ) {
			$errors->add(
				...
				__( 'The account password is required to continue.', 'flexify-checkout-for-woocommerce' ),
	...
	public function validate_shipping_address( $data, $errors ) {
		...
		if ( ! $has_successful_response ) {
			$errors->add( 'invalid_postcode',
				__( 'Invalid ZIP code.', 'flexify-checkout-for-woocommerce' ),
				[ 'id' => 'billing_postcode' ]
			);
		...
		if ( ! $city_matches || ! $state_matches ) {
			$errors->add( 'invalid_postcode_region',
				__( 'The provided ZIP code does not belong to the selected city/state.', 'flexify-checkout-for-woocommerce' ),
				[ 'id' => 'billing_postcode' ]
			);
		}
	}
