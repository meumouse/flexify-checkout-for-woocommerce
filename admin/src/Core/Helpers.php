...
class Helpers {
	 ...
	 *
	 * Active when the "Swift" checkout theme is selected and the license is
	 * valid. When it returns false the classic server-rendered checkout is
	 * served unchanged, so a lapsed license safely falls back to the legacy
	 * checkout instead of leaving the store without a checkout.
	 *
	 * @since 6.0.0
	 * @return bool
	 */
	public static function is_react_checkout_enabled() {
		...
		// the React checkout.
		$enabled = Admin_Options::get_setting('flexify_checkout_theme') === 'swift' && License::is_valid();
