	...
	public function add_new_field_to_checkout_callback() {
		if ( isset( $_POST['get_field_id'] ) ) {
			...
			if ( ! isset( $get_fields[$field_id] ) ) {
				...
				if ( $field_added ) {
					ob_start();
					Admin_Options::render_field( $field_id, $new_field[$field_id], $new_field[$field_id]['step'] );
...
if ( ! class_exists('MeuMouse\Flexify_Checkout\Ajax\Ajax') ) {
    class_alias( 'MeuMouse\Flexify_Checkout\Ajax', 'MeuMouse\Flexify_Checkout\Ajax\Ajax' );
}