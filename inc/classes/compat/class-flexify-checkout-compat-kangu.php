...
if ( class_exists( 'Flexify_Checkout_Compat_Kangu' ) && ! class_exists('Kangu_Shipping_Method') ) {
    return;
}
 ...
 */
class Flexify_Checkout_Compat_Kangu {
    ...
    public static function run() {
        ...
        add_filter( 'woocommerce_package_rates', array( __CLASS__, 'fix_kangu_shipping_methods' ), 10, 2 );
        ...
        add_action( 'woocommerce_after_shipping_rate', array( __CLASS__, 'add_extra_shipping_details' ), 10, 1 );

        if ( Flexify_Checkout_Init::get_setting('enable_display_local_pickup_kangu') === 'yes' ) {
            add_action( 'woocommerce_after_shipping_rate', array( __CLASS__, 'display_local_pickup_kangu' ) );
        }

        add_filter( 'woocommerce_cart_shipping_method_full_label', array( __CLASS__, 'format_names_for_kangu_shipping_method' ), 10, 2 );
    }
     ...
     * @return array
     */
    public static function fix_kangu_shipping_methods( $rates, $package ) {
     ...
     * For pick-up points, I display a block with the address and other information.
     * 
     * @since 3.3.0
     * @param object $method | Shipping method
     * @return void
     */
    public static function add_extra_shipping_details( $method ) {
        $meta = $method->get_meta_data();
    
        if ( isset( $meta['point_address'] ) ) {
            printf( '<div class="point-address"><div class="name">(Ponto Kangu) %s</div><div class="address">%s</div><div class="distance">%s</div></div>', 
     ...
     * @return void
     */
    public static function display_local_pickup_kangu( $method ) {
        if ( $method->method_id == 'local_pickup' ) {
            printf( '<div class="point-address"><div class="address">%s %s, %s, %s</div></div>',
     ...
     * @return string
     */
    public static function format_names_for_kangu_shipping_method( $label, $method ) {
