...
$mobile_show_summary = Admin_Options::get_setting('display_opened_order_review_mobile') === 'yes' ? true : false;
...
if ( ! $checkout->is_registration_enabled() && $checkout->is_registration_required() && ! is_user_logged_in() ) :
	 ...
	 * @since 1.0.0
	 */
	$message = apply_filters( 'woocommerce_checkout_must_be_logged_in_message', __( 'You need to be logged in to complete your purchase.', 'flexify-checkout-for-woocommerce' ) );
	$message = sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ), __( 'Enter', 'flexify-checkout-for-woocommerce' ), $message ); ?>
...
<button type="button" class="flexify-checkout__sidebar-header">
	<div class="flexify-checkout__sidebar-header-inner">
		<span class="flexify-checkout__sidebar-header-link">
			...
			<span class="flexify-checkout__sidebar-header-link--show" style="<?php echo $mobile_show_summary ? 'display:none' : ''; ?>">
				<?php esc_html_e( 'Show order summary', 'flexify-checkout-for-woocommerce' ); ?>
			</span>
			
			<span class="flexify-checkout__sidebar-header-link--hide" style="<?php echo $mobile_show_summary ? '' : 'display:none'; ?>">
				<?php esc_html_e( 'Hide order summary', 'flexify-checkout-for-woocommerce' ); ?>
			</span>
		</span>
